...
- **HUPy state file** (`hupy-state.json`) — new `hupy.state` package: `HupyStateFile` schema (`logger_verbosity`, `skip_once`), resolved via `get_state_file_path(repo)` inside the repo's `.git` directory; `open_state_file(repo)` opens and atomically saves it back, thread- and process-safe via a file lock plus a `tempfile`/`os.replace` write; a missing file falls back to schema defaults
- **`skip-once`/`s`** CLI subcommand — flags one or more modules (`vg`/`ttg`/`pch`/`bdc`/`hb`, or their full names like `ver-grep`/`triage-tag-gating`) to be skipped for exactly the next `pre-commit`/`prepare-commit-msg` run, recorded in `hupy-state.json`'s `skip_once` set
- **centralized module run-gating** — new `should_run_module(repo, state_file, module_abbr)` combines a module's config `is_disabled` flag with a one-time `skip_once` flag (checked via `HupyStateFile.consume_skip_once`, which no longer discards the flag on check — it stays set for every call across a round, since a module like `hb` is checked multiple times per round; `HupyStateFile.clear_skip_once` spends the whole set once the round finishes, called at the end of `prepare-commit-msg`'s dispatch) into a single decision; `is_disabled` is checked first and short-circuits without consuming `skip_once`; `ban_direct_commit`, `perform_triage_tags_gating`, and `prepend_commit_header` each take a new `state_file` parameter and call it in place of their own former direct `is_disabled` check
...
- renamed `write_default_config` → `create_default_config_file` (now takes a `git.Repo`, not a bare `repo_root` path); renamed `hupy/config/hupy_config_file.py` → `hupy/config/config_file.py`
...
- `default_logger_verbosity` removed from `HupyConfigFile`; verbosity now comes from `hupy-state.json`'s `logger_verbosity` field instead, read by `pre-commit`/`prepare-commit-msg` through the new `open_state_file` context
...
- config path constants/helpers (`CONFIG_FILENAME`, `DEFAULT_CONFIG_ASSET`, `get_config_file_path`) extracted out of `load_config.py`/`write_config.py` into a new `hupy/config/config_file_path.py`
