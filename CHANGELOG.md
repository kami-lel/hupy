...
- **`skip-once`/`s`** CLI subcommand — flags one or more modules (`vg`/`ttg`/`pch`/`bdc`/`hb`, or their full names like `ver-grep`/`triage-tag-gating`) to be skipped for the next commit round, recorded in `hupy-state.json`'s `skip_once` set; the new `-u`/`--unset` flag removes the flag(s) instead of setting them
