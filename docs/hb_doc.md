...
See the [Hook Chain](chain_doc.md) for where each *lead*/*trail* pair sits relative to [Ban Direct Commit](bdc_doc.md), [Triage Tag Gating](ttg_doc.md), and [Prepend Commit Header](pch_doc.md). An HB entry's `allow_commit_types` filters on the commit and merge types defined in [Commit, Branch & Merge](cbm_doc.md).
