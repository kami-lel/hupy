...
Use it the **first time** HUPy is adopted in a repository (eg right after cloning one that uses HUPy), and again any time demand may have drifted — it's convergent, so a repeat run is safe: files already correct are left alone, missing ones are written, and a stub that's drifted from what HUPy renders, or one no longer demanded, is only touched under `-f`/`--force` or `--prune` respectively.
