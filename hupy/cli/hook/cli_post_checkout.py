"""
cli_post_checkout.py

define ``register_cli_post_checkout_parser``, the post-checkout hook
subcommand
"""
...
_POST_CHECKOUT_DOC = "run post-checkout stage hooks"
...
def _post_checkout_main(args):  ################################################
    """
    dispatch for the ``post-checkout`` subcommand
    """
...
# Public API  ##################################################################
def register_cli_post_checkout_parser(subparser):
    """
    register the ``post-checkout`` subcommand parser.
    """
    post_checkout_parser = subparser.add_parser(
        _HOOK_NAME,
        help=_POST_CHECKOUT_DOC,
        description=_POST_CHECKOUT_DOC,
    )
    ...
    kamilog.add_verbose_arguments(post_checkout_parser)
    post_checkout_parser.set_defaults(func=_post_checkout_main)
