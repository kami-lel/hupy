...
define ``register_cli_pre_auto_gc_parser``, the pre-auto-gc hook
subcommand
"""
...
# constants  ###################################################################
_HOOK_NAME = "pre-auto-gc"
_PRE_AUTO_GC_DOC = "run pre-auto-gc stage hooks"
...
def _pre_auto_gc_main(args):  ##################################################
    """
    dispatch for the ``pre-auto-gc`` subcommand
    """
...
# Public API  ##################################################################
def register_cli_pre_auto_gc_parser(subparser):
    """
    register the ``pre-auto-gc`` subcommand parser.
    """
    pre_auto_gc_parser = subparser.add_parser(
        _HOOK_NAME,
        help=_PRE_AUTO_GC_DOC,
        description=_PRE_AUTO_GC_DOC,
    )
    pre_auto_gc_parser.add_argument(
        "hook_args",
        nargs="*",
        help="raw arguments forwarded",
    )
    kamilog.add_verbose_arguments(pre_auto_gc_parser)
    pre_auto_gc_parser.set_defaults(func=_pre_auto_gc_main)
