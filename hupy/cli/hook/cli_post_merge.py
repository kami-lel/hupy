...
define ``register_cli_post_merge_parser``, the post-merge hook
subcommand
"""
...
_POST_MERGE_DOC = "run post-merge stage hooks"
...
def _post_merge_main(args):  ###################################################
    """
    dispatch for the ``post-merge`` subcommand
    """
...
# Public API  ##################################################################
def register_cli_post_merge_parser(subparser):
    """
    register the ``post-merge`` subcommand parser.
    """
    post_merge_parser = subparser.add_parser(
        _HOOK_NAME,
        help=_POST_MERGE_DOC,
        description=_POST_MERGE_DOC,
    )
    ...
    kamilog.add_verbose_arguments(post_merge_parser)
    post_merge_parser.set_defaults(func=_post_merge_main)
