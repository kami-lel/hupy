...
define ``register_cli_pre_rebase_parser``, the pre-rebase hook
subcommand
"""
...
# constants  ###################################################################
_HOOK_NAME = "pre-rebase"
_PRE_REBASE_DOC = "run pre-rebase stage hooks"
...
def _pre_rebase_main(args):  ###################################################
    """
    dispatch for the ``pre-rebase`` subcommand
    """
...
# Public API  ##################################################################
def register_cli_pre_rebase_parser(subparser):
    """
    register the ``pre-rebase`` subcommand parser.
    """
    pre_rebase_parser = subparser.add_parser(
        _HOOK_NAME,
        help=_PRE_REBASE_DOC,
        description=_PRE_REBASE_DOC,
    )
    pre_rebase_parser.add_argument(
        "hook_args",
        nargs="*",
        help="raw arguments forwarded",
    )
    kamilog.add_verbose_arguments(pre_rebase_parser)
    pre_rebase_parser.set_defaults(func=_pre_rebase_main)
