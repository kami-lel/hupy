...
- **`tests/ttg/prep_repo.py`** — shared between tests and both `examples/pch/*.py` and `examples/ttg/*.py` demos, all of which `sys.path.insert`-import `prepare_repo(dest_dir, scenario)` directly rather than shelling out; also runnable standalone via `--scenario <name>`, printing the prepared repo path
