...
- **`_Ttg(BaseModel)`** — `is_disabled: bool`, `disable_tt_detect_by_type: bool`, `ignored_path_globs: list[str]`; new this round. Only `is_disabled` is consumed so far, by `ttg.tt_gating.perform_triage_tags_gating` (see `ttg.tt_gating` in Module Details); `disable_tt_detect_by_type` and `ignored_path_globs` are schema-only, not yet read by `tt_detect`/`tt_gating`
