...
- **`_Bdc(BaseModel)`** — `ban_commit_to_dev: bool = True`, `ban_commit_to_main: bool = True`, `ban_commit_to_branches: list[str] = Field(default_factory=list)`; consumed by `bdc.ban_direct_commit._get_protected_branches` (see `bdc` in Module Details) — documented in `docs/hupy_config_doc.md`'s `bdc` section
