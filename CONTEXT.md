...
| `pch` | **implemented** | prepend header lines to in-progress commit messages for Feature Landing and Version Release merges; the latter appends a version number via `ver_grep` when configured |
...
- level 2 (under `MERGE`): `FEATURE_LANDING` | `VERSION_RELEASE` | `OTHER_MERGE`
