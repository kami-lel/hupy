...
examples/
  hooks/                           # bash demos driving the real `hupy <stage>` CLI end-to-end
    pre-commit-demo.sh             # Feature Finish merge, steady+quiet TT only; expects PASS
...
- **`tests/fixtures/prep_repo.py`** — the shared repo-scenario builder, used by `tests/pch/`, `tests/ttg/` unit tests and by both `examples/pch/*.py` and `examples/ttg/*.py` demos (all `sys.path.insert` onto `tests/fixtures/` and import directly rather than shelling out). Exposes two builders: `prepare_repo(dest_dir, scenario)` for the TTG/PCH `SCENARIOS` that unit tests also exercise (`non_merge_commit`, `irrelevant_merge`, `feature_finish_{pass,fail}`, `version_release_{pass,fail}`), and `prepare_demo_repo(dest_dir, demo_bucket)` for the demo-only `DEMO_BUCKETS` covering CBM merge types PCH doesn't handle yet (`sync_backport`, `catch_up`, `hotfix_release`, `hotfix_backport`, `release_cut`, `release_backport`) — the latter has no unit-test caller since there's nothing to assert beyond "PCH skips it". Also runnable standalone as a CLI with mutually exclusive `--scenario <name>` / `--demo-bucket <name>` flags plus `--dest`, printing the prepared repo path; `examples/prepare-commit-msg-demo.sh` shells out to this CLI rather than re-implementing repo setup in bash.
