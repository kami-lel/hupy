...
Each utility is a standalone module in the `hupy/` package, callable independently from any git hook script. No cross-module dependencies are assumed, except within the `ttg` package itself; `pch`'s/`ttg`'s/`bdc`'s use of `cbm`; `cbm`'s/`ver_grep`'s/`bdc`'s use of `config`; and `bdc`'s/`ttg`'s/`pch`'s use of the top-level `should_run_module`, which itself depends on both `config` and `state`.
...
| `config` | **implemented** | `HupyConfigFile` pydantic schema for `.hupy.config.jsonc` (including the nested `vg` and `cbm` sections), cached loading (as JSON5) resolved against an already-open `git.Repo`, and default-config-copying helpers |
...
**Public API**: `CONFIG_LOGGER_NAME` (`hupy/config/__init__.py`, `= "HU.config"`) · `HupyConfigFile` (`config_file.py`, renamed from `hupy_config_file.py`/`HupyConfig`/`model.py`) · `CONFIG_FILENAME`, `DEFAULT_CONFIG_ASSET`, `get_config_file_path(repo)` (`config_file_path.py`) · `load_hupy_config(repo)` (`load_config.py`) · `create_default_config_file(repo, force)` (`write_config.py`, renamed from `write_default_config(repo_root, force)`)
...
- **`HupyStateFile.consume_skip_once(module_abbr)`** — checks `module_abbr` against `skip_once`, `discard`s it if present, and returns whether it was found; called by `should_run_module` (see below) so a flag set by `skip-once` is spent by exactly the next check that consults it
...
2. `state_file.consume_skip_once(module_abbr)` — only reached if step 1 didn't already return `False`; this ordering means a config-disabled module never consumes a pending `skip_once` flag — the flag stays queued for a future run where the module is actually enabled
...
**Known gap**: only `bdc`, `ttg`, and `pch` actually call `should_run_module`. `vg`/`hb` are listed in `cli_skip_once.SKIPPABLE_MODULE` and have entries in this module's own `_MODULE_ABBR_TO_NAME`, so `hupy skip-once vg`/`hupy skip-once hb` succeed and write to `skip_once` without error — but nothing ever calls `should_run_module(repo, state_file, "vg")` or `"hb"` to consume that flag, so flagging either module is currently a silent no-op. `vg` (`ver_grep`) has no standalone CLI entry point of its own to gate — it's only ever invoked internally by `pch` — and `hb` has no implementation at all yet (see Status line in Project Overview).
...
- **`prepare-commit-msg`** (`cli_prepare_commit_msg.py`) — same `git.Repo`-build-then-`open_state_file`-then-verbosity pattern, then calls `prepend_commit_header(repo, state_file)`; logs entry/exit via the logger (no nested subcommands)
...
hupy/                             # installable package
  ...
  state/                          # HUPy state schema and file writing (hupy-state.json)
    ...
    state_file.py                  # HupyStateFile pydantic model: logger_verbosity, skip_once; consume_skip_once(module_abbr)
...
- **`tests/state/state-consume_skip_once_test.py`** — direct unit tests on `HupyStateFile.consume_skip_once`, no config/repo fixtures needed: not-flagged returns `False` without mutating `skip_once`, flagged returns `True` and discards just that entry (siblings untouched), and a second call for the same `module_abbr` returns `False` (idempotent).
- **`tests/should_run_module/should_run_module_test.py`** — patches `hupy.should_run_module.load_hupy_config` (a separate patch target from each consuming module's own `load_hupy_config` reference, since Python binds `from ... import` at each call site) and constructs `HupyStateFile` instances directly, parametrized across all 5 entries in `should_run_module`'s `_MODULE_ABBR_TO_NAME`. Asserts: enabled + unflagged runs; `is_disabled` blocks regardless of `skip_once`; `skip_once` blocks and is consumed (only the matching entry, siblings left alone); `is_disabled` is checked first and short-circuits *without* consuming a pending `skip_once` flag; and a second call after consumption runs normally (flag already spent).
