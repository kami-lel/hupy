...
| `pch` | **implemented** | prepend header lines to in-progress commit messages for Feature Landing and Stable Release merges; the latter appends a version number via `ver_grep` when configured |
...
- level 2 (under `MERGE`): `FEATURE_LANDING` | `STABLE_RELEASE` | `OTHER_MERGE`
...
4. source branch ≠ `MAIN_BRANCH` and target == `DEV_BRANCH` → `FEATURE_LANDING`
5. source == `DEV_BRANCH` and target == `MAIN_BRANCH` → `STABLE_RELEASE`
...
- `FEATURE_LANDING` merge → `"Feature Landing: <source-branch-name>"` + blank line + original message
- `STABLE_RELEASE` merge → `_gen_stable_release_header_content()` calls `ver_grep.grep_repo_version()`; if it returns a non-empty version string, the header is `"Stable Release: <version>"`, otherwise plain `"Stable Release"` (when `ver_grep` is unconfigured — see `ver_grep` in Module Details) + blank line + original message
...
README flags a planned scenario not yet covered by `examples/pch/`/`tests/pch/`: keeping a feature branch up to date by merging `dev` back *into* it (as opposed to the Feature Landing direction, feature → `dev`).
...
Extracts a repo's version string by regex-matching a line in a configured version file; consumed by `pch` to append a version number to the Stable Release commit header.
...
examples/
  ...
  pch/                            # 10 runnable PCH demo scripts (passes & skips), .py
  ttg/                            # 6 runnable TTG demo scripts (fail/pass/skip), .py
tests/
