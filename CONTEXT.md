...
*Last updated: 2026-07-18 (later same day). This file describes the current architecture, not its evolution — for the full change history see `CHANGELOG.md`.*
...
- **`grep_occurrence(repo, ref, occurrence)`** (`ver_grep.py`, not re-exported) — reads `occurrence.file` at `ref` via `repo.git.show(f"{ref}:{occurrence.file}")`, returns the first capturing-group match of `occurrence.glob` against its lines. File missing at `ref`, no matching line, or a match with no capture group → `warning` + return `""`. Shared by `grep_version` (against `version_occurrences[0]`) and `check_version_uniformity` (against every later entry).
