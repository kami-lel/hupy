...
`examples/pch/*.py` and `examples/ttg/*.bash` remain demo/test scripts (see Testing Infrastructure) run standalone for scenario walkthroughs — they are not, and were never meant to be, install-ready hook scripts, and this is unaffected by the redesign.
...
examples/
  pch/                            # 4 runnable PCH demo scripts (passes & skips), .py
...
- **`tests/ttg/prep_repo.py`** — shared between tests and both `examples/pch/*.py` and `examples/ttg/*.bash` demos (the `.py` demos `sys.path.insert`-import `prepare_repo(dest_dir, scenario)` directly rather than shelling out); also runnable standalone via `--scenario <name>`, printing the prepared repo path so bash demos can `cd` into it
